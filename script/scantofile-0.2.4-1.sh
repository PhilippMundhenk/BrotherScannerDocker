...
(
	...
	(
		...
		/opt/brother/scanner/brscan-skey/script/trigger_telegram.sh "Page front converted"
		...
		if [ -z "${OCR_SERVER}" ] || [ -z "${OCR_PORT}" ] || [ -z "${OCR_PATH}" ]; then
			...
			(
				...
				/opt/brother/scanner/brscan-skey/script/trigger_telegram.sh "Page front OCR finished"
